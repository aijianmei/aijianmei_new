...
- (UIBarButtonItem *)itemButtonWithTitle:(NSString *)title selector:(SEL)seletor
{
    UIBarButtonItem *doneButton = [[[UIBarButtonItem alloc] initWithTitle:title style:UIBarButtonItemStylePlain target:self action:seletor] autorelease];
    // Set appearance
...
//            [doneButton setTitleTextAttributes:[NSDictionary dictionary] forState:UIControlStateHighlighted];
//        }
        self.navigationItem.leftBarButtonItem = [self itemButtonWithTitle:NSLocalizedString(@"kDone", nil) selector:@selector(doneButtonPressed:)];
        if (_canSave) {
            self.navigationItem.rightBarButtonItem = [self itemButtonWithTitle:NSLocalizedString(@"kSave", nil) selector:@selector(savePhoto:)];
        }
...
- (void)savePhoto {
    id <MWPhoto> photo = [self photoAtIndex:_currentPageIndex];
    if ([photo underlyingImage]) {
        [self showProgressHUDWithMessage:[NSString stringWithFormat:@"%@\u2026" , NSLocalizedString(@"kSaving", @"Displayed with ellipsis as 'Saving...' when an item is in the process of being saved")]];
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    [self showProgressHUDCompleteMessage: error ? NSLocalizedString(@"kFailed", @"Informing the user a process has failed") : NSLocalizedString(@"kSaved", @"Informing the user an item has been saved")];
    [self hideControlsAfterDelay]; // Continue as normal...
}
